...
def number_of_terms(gs1, gs2):
    """
    Given two generating series, this calculates the number of branches present
    in the reduction tree (the number of terms in the shuffle expansion).
    
    This is used in the caching as we need to know if all the terms for a
    generating series have been cached.
    """
    # Need to account for the coefficient in the zeroth column.
    l1 = len(gs1) - 1
    l2 = len(gs2) - 1
    
    return int(factorial(l1 + l2) / (factorial(l1) * factorial(l2)))
...
def subshuffle_cacher(func):
    ...
    subshuf_cache = defaultdict(lambda: [0, []])
    
    # cache_count keeps track of the number of terms that have been cached for
    # each key. When all terms for a generating series expansion have been
    # cached, the wrapper will begin to lookup the values.
    cache_count = defaultdict(int)
    
    # Only cache if the shuffle expansion makes more than CACHE_CUT_IN terms.
    CACHE_CUT_IN = 5
    
    @functools.wraps(func)
    def wrapper(gs1, gs2, term_stack, *stor_args):
        ...
        num_children = number_of_terms(gs1, gs2)
        
        if num_children < CACHE_CUT_IN:
            # Only cache when it's worthwhile.
            return func(gs1, gs2, term_stack, *stor_args)
        
        elif (cache_count[key] < num_children):
            state_diffs, final_states = func(gs1, gs2, term_stack, *stor_args)
            
            cache_count[key] += len(state_diffs)
            subshuf_cache[key][0] = coeff
            subshuf_cache[key][1].extend(state_diffs)
...
@subshuffle_cacher
def subshuffle(gs1, gs2, *term_args):
    """
...
def reduce_gs(gs1, gs2, term_stack, *stor_args):
    """
...
def handle_gs_constants(gs1, gs2, term_stack, coefficient_count, term_storage):
    """
...
def stack_difference(final_states, current_term_stack):
    """
